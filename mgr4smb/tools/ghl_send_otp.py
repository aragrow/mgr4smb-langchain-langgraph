...
def ghl_send_otp(contact_email: str, contact_phone: str) -> str:
    ...
    try:
        ...
        # Always generate a fresh code when this tool is called. Whatever
        # was previously stored on the contact (from an earlier session, an
        # expired code, etc.) is replaced. Session-level "send once" is
        # enforced upstream by the OTP_AGENT prompt — it checks the
        # conversation history for a prior OTP_SENT before deciding to
        # call this tool. Putting the policy here would conflate sessions
        # since the GHL contact record is shared across all of them.
