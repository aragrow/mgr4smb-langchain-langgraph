...
def fetch_contact(contact_id: str) -> dict:
    ...
    index that lags behind PUTs by several seconds, which broke OTP
    ...
    data = resp.json()
    # GHL wraps the contact in a "contact" envelope on this endpoint
...
#
# This cache resolves a key (e.g. "contact.otp_code" or "otp_code") to the
# field id once per process, so subsequent OTP send/verify calls skip the
...
def resolve_custom_field_id(key_or_id: str) -> str:
    ...
    Caches the resolution per process. Raises GHLAPIError if the key is not
    found in the location's custom-fields catalogue.
    """
    ...
    # Look it up against the location's custom field definitions
    try:
        resp = get_client().get(
